...
class AttendanceFilterView extends GetView<AttendanceController> {
  ...
  Widget _buildResponsiveTablefilter(BuildContext, context) {
    ...
    return isMobile
        ? _buildAttendanceTablefilterMobile(context!)
        : _buildAttendanceTablefilterother(
          controller: controller,
          selectedFilter: controller.selectedFilter,
          selectedExport: controller.selectedExport,
        );
  }
                    ...
                    if (newValue != null) {
                      ...
                      if (newValue == 'PDF') {
                        await Future.microtask(
                          () =>
                              ExportPDF(attendaData: controller.attendanceData),
                        );
                      } else if (newValue == 'EXCEL') {
                        await Future.microtask(
                          () => exportToExcel(
                            attendaData: controller.attendanceData,
                          ),
                        );
