...
class EmployeeReportController3 extends GetxController {
  ...
  Future<void> fetchData({required int page, required int pageSize}) async {
    ...
    try {
      ...
      final newData = await employeeabsentsql.employeeabsent(
      ...
      if (page == 1) {
        data.assignAll(
          newData.map((e) => EmployeeAbsentModel.fromJson(e)).toList(),
        );
      } else {
        data.addAll(
          newData.map((e) => EmployeeAbsentModel.fromJson(e)).toList(),
